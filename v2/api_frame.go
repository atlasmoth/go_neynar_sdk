...
type ApiFrameFromUrlRequest struct {
	ctx context.Context
	ApiService *FrameAPIService
	apiKey *string
	url *string
...
// API key required for authentication.
func (r ApiFrameFromUrlRequest) ApiKey(apiKey string) ApiFrameFromUrlRequest {
	r.apiKey = &apiKey
	return r
}

// The frame URL to crawl
func (r ApiFrameFromUrlRequest) Url(url string) ApiFrameFromUrlRequest {
	r.url = &url
	return r
}

func (r ApiFrameFromUrlRequest) Execute() (*FrameFromUrl200Response, *http.Response, error) {
	return r.ApiService.FrameFromUrlExecute(r)
}

/*
FrameFromUrl Fetches the frame meta tags from the URL

Fetches the frame meta tags from the URL

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiFrameFromUrlRequest
*/
func (a *FrameAPIService) FrameFromUrl(ctx context.Context) ApiFrameFromUrlRequest {
	return ApiFrameFromUrlRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
//  @return FrameFromUrl200Response
func (a *FrameAPIService) FrameFromUrlExecute(r ApiFrameFromUrlRequest) (*FrameFromUrl200Response, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *FrameFromUrl200Response
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FrameAPIService.FrameFromUrl")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
